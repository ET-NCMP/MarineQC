...
class test_sphere_distance(unittest.TestCase):
...
class test_lat_lon_from_course_and_distance(unittest.TestCase):
...
class test_course_between_points(unittest.TestCase):
...
class test_intermediate_points(unittest.TestCase):

    def test_equator_is_halfway_from_pole_to_pole(self):
        '''make sure equator is halfway between (almost) the poles'''
        lat,lon = intermediate_point(-89,0,89,0,0.5) #don't do 90S to 90N as the great circle is not uniquely defined
        self.assertEqual(0.0,lat)
        self.assertEqual(0.0,lon)
