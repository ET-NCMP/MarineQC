...
class Test_old_buddy(unittest.TestCase):
...
class Test_get_threshold_multiplier(unittest.TestCase):
...
class Test_MarineReport(unittest.TestCase):

    def setUp(self):

        vals = [{'ID':'AAAAAAAAA', 'YR':2003, 'MO':12, 'DY':1, 'HR':0,    'LAT':0.0, 'LON':0.0, 'SST':5.0, 'AT':10.0, 'DPT':11.0, 'SLP':1023.0, 'UID':'A642D2'},
                {'ID':'AAAAAAAAA', 'YR':2003, 'MO':12, 'DY':1, 'HR':0,    'LAT':0.0, 'LON':0.0, 'SST':5.0, 'AT':10.0, 'DPT':10.0},
                {'ID':'BBBBBBBBB', 'YR':2003, 'MO':12, 'DY':1, 'HR':0,    'LAT':0.0, 'LON':0.0, 'SST':5.0, 'AT':10.0, 'DPT':9.0},
...
class Test_QC_filter(unittest.TestCase):
    ...
    def test_creation(self):
        ...
        filter = ex.QC_filter()
        self.assertEqual(filter.test_report(self.rep),0)

    def test_split(self):
        
        filter = ex.QC_filter()
        filter.add_qc_filter('POS','pos',0)
        
        p,f = filter.split_reports(self.deck)
    ...
    def test_fail(self):
        ...
        self.assertEqual(filter.test_report(self.rep_fail),1)
...
class Test_bayesian_buddy(unittest.TestCase):
