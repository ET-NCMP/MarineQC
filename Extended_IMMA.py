...
class MarineReport:
    ...
    def calculate_humidity_variables(self, hum_vars):
        ...
        assert all([i in ['SHU', 'VAP', 'CRH', 'CWB', 'DPD'] for i in
                    hum_vars]), 'Not all hum vars are present: SHU,VAP,CRH,CWB,DPD'
    ...
    def calculate_dt(self):
        ...
        if self.getvar('YR') is not None:
            ...
            if (self.getvar('MO') is not None and
                    ...
                    0 < self.getvar('DY') <= mlen[self.getvar('MO') - 1]):
    ...
    def get_normalised_anom(self, varname):
        ...
        if varname in self.climate_variables:
            ...
            if self.getvar(varname) is not None and clim is not None and stdev is not None:
    ...
    def saturated(self):
        ...
        if (self.getvar('DPT') == self.getvar('AT') and
                self.getvar('AT') is not None and
                self.getvar('DPT') is not None):
            return True
        else:
            return False
...
class MarineReportQC(MarineReport):
    ...
    def do_base_slp_qc(self, parameters):
        ...
        self.set_qc('SLP', 'clim', qc.climatology_plus_stdev_with_lowbar(self.getvar('SLP'),
                                                                         self.getnorm('SLP'),
...
class Voyage:
    ...
    def spike_check(self, parameters, intype='SST'):
        ...
        for t1 in range(0, numobs):
            ...
            for t2 in range(lo, hi):
                ...
                if (self.getvar(t2, intype) is not None and
                        self.getvar(t1, intype) is not None):
    ...
    def buoy_aground_check(self, parameters, sort=True):
        ...
        if sort:
            self.sort()
        try:
            tqc.aground_check(self.reps, parameters['smooth_win'], parameters['min_win_period'],
    ...
    def new_buoy_aground_check(self, parameters, sort=True):
        ...
        if sort:
            self.sort()
        try:
            tqc.new_aground_check(self.reps, parameters['smooth_win'], parameters['min_win_period'])
        except AssertionError as error:
            raise AssertionError('buoy aground check failed to run: ' + str(error))
        return
    ...
    def buoy_speed_check(self, parameters, sort=True):
        ...
        if sort:
            self.sort()
        try:
            tqc.speed_check(self.reps, parameters['speed_limit'], parameters['min_win_period'],
                            parameters['max_win_period'])
        except AssertionError as error:
    ...
    def new_buoy_speed_check(self, iquam_parameters, parameters, sort=True):
        ...
        if sort:
            self.sort()
        try:
            tqc.new_speed_check(self.reps, iquam_parameters, parameters['speed_limit'], parameters['min_win_period'])
        except AssertionError as error:
            raise AssertionError('buoy speed check failed to run: ' + str(error))
        return
    ...
    def buoy_tail_check(self, parameters, sort=True):
        ...
        if sort:
            self.sort()
        try:
            tqc.sst_tail_check(self.reps, parameters['long_win_len'], parameters['long_err_std_n'],
                               parameters['short_win_len'],
                               parameters['short_err_std_n'], parameters['short_win_n_bad'], parameters['drift_inter'],
                               parameters['drif_intra'], parameters['background_err_lim'])
        except AssertionError as error:
    ...
    def buoy_bias_noise_check(self, parameters, sort=True):
        ...
        if sort:
            self.sort()
        try:
            tqc.sst_biased_noisy_check(self.reps, parameters['n_eval'], parameters['bias_lim'],
                                       parameters['drif_intra'], parameters['drif_inter'],
                                       parameters['err_std_n'], parameters['n_bad'],
    ...
    def write_tracking_output(self, runid, icoads_dir, y1, m1, y2, m2):
        ...
        header2 = self.reps[0].print_qc_block('POS', qc_block1, header=True, printuid=False)
        header3 = self.reps[0].print_qc_block('SST', qc_block2, header=True, printuid=False)
        header4 = self.reps[0].print_qc_block('POS', qc_block3, header=True, printuid=False)
        header5 = self.reps[0].print_qc_block('SST', qc_block4, header=True, printuid=False)
        header6 = self.reps[0].print_variable_block(varnames2, header=True, printuid=False)

        outfile.write(header1.rstrip() + "," + header2.rstrip() + "," + header3.rstrip() + "," +
                      header4.rstrip() + "," + header5.rstrip() + "," + header6)
        ...
        for rep in self.reps:
            ...
            outfile.write(vars1.rstrip() + "," + vars2.rstrip() + "," + vars3.rstrip() + "," +
                          vars4.rstrip() + "," + vars5.rstrip() + "," + vars6)
    ...
    def write_output(self, runid, icoads_dir, year, month):
        ...
        # write out base QC
        allvarnames = {
            'POS': ['day', 'isship', 'trk', 'date', 'time', 'pos', 'blklst', 'isbuoy', 'iquam_track', 'isdrifter'],
    ...
    def get_buddy_limits_with_parameters(self, pentad_stdev, limits,
        ...
        for i in range(len(nonmiss[0])):
            ...
            for j, limit in enumerate(limits):
                ...
                temp_anom, temp_nobs = self.get_neighbour_anomalies(limits[j],
                                                                    xindex,
                                                                    yindex,
                                                                    pindex)
